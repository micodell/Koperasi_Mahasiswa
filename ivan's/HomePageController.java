...
//    }
    ...
    public void update_from_table(String table) throws SQLException {
        ...
        try {
            switch (table) {
                case "Departments" -> {
                    String update_dept_query = "UPDATE Departments SET name_department = ?, jobdesc = ? WHERE id_department = ?";
                    st = con.prepareStatement(update_dept_query);
                ...
                case "Roles" -> {
                    String update_roles_query = "UPDATE Roles SET name_role = ? WHERE id_department = ?";
                    st = con.prepareStatement(update_roles_query);
                ...
                case "Employees(Pegawai)" -> {
                    String update_roles_query = "UPDATE Employees SET nama_pegawai = ? WHERE id_pegawai = ?";
                    st = con.prepareStatement(update_roles_query);
                ...
                case "Executives(Anggota)" -> {
                    String update_roles_query = "UPDATE Executives SET nama_anggota = ? WHERE id_anggota = ?";
                    st = con.prepareStatement(update_roles_query);
                    ...
                    int rowsUpdated = st.executeUpdate();
                    if (rowsUpdated > 0) {
                        System.out.println("Executives updated successfully.");
                    }

                    view_table(table);
                }
                case "Stocks(Barang)" -> {
                    String update_roles_query = "UPDATE Stocks SET nama_barang = ?, harga_pokok = ?, harga_jual = ? WHERE id_barang = ?";
                    st = con.prepareStatement(update_roles_query);

                    st.setString(1, textField2.getText());
                    st.setInt(2,Integer.parseInt(textField3.getText()));
                    st.setInt(3,Integer.parseInt(textField4.getText()));
                    st.setInt(4, Integer.parseInt(textField1.getText()));
                ...
                case "Categories(Kategori)" -> {
                    String update_roles_query = "UPDATE Categories SET nama_kategori WHERE id_kategori = ?";
                    st = con.prepareStatement(update_roles_query);
                ...
                case "Transactions" -> {
                    String update_roles_query = "UPDATE Transactions SET tanggal_transaksi = ?, nama_seller = ?, harga_jual = ? WHERE id_barang = ?";
                    st = con.prepareStatement(update_roles_query);
